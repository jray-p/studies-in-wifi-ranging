...
copied from example here: https://feitcsi.kuskosoft.com/python/
modified by jray.
'''
...
def parseFeitCSI(inbytes: bytes) -> list:

    step = 0
    output = []
    while (len(inbytes) > step):
        data = {}
        data["header"] = parseHeader(inbytes[step:(step+272)])
        step += 272
        data["csi_matrix"] = parseCsiData(inbytes[step:(step + data["header"]["csi_size"])], data["header"])
