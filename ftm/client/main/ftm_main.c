                          ...
                          int32_t event_id, void *event_data)
{
	if (event_id == WIFI_EVENT_STA_CONNECTED) {
        ...
        s_rssi_est = event->ftm_report_data[0].rssi;
...
#error Unsupported console type
    ...
    ESP_ERROR_CHECK(uart_driver_install(2, 1024 * 2, 0, 0, NULL, 0));
    ESP_ERROR_CHECK(uart_param_config(2, &uart_config_out));
    ESP_ERROR_CHECK(uart_set_pin(2, 4, 5, -1, -1));


    wifi_cmd_sta_join("mahftmap", "theverybestpassword");
    char *args[] = {
        "-I",
        "-s",
        "mahftmap",
    ...
    while (1) {
      ...
      uart_write_bytes(2, (const char *)&s_rtt_est, sizeof(s_rtt_est));
      uart_write_bytes(2, (const char *)&s_rtt_raw, sizeof(s_rtt_raw));
      uart_write_bytes(2, (const char *)&s_rssi_est, sizeof(s_rssi_est));
      uart_write_bytes(2, (const char *)&s_dist_est, sizeof(s_dist_est));
      uart_flush(2);
    }
